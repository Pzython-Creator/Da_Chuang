...
class FFPP_Dataset(data.Dataset):
    ...
    def find_video_file(self,directory, extensions):  
        """  
        在指定目录中寻找具有指定扩展名的视频文件，并返回第一个找到的文件名（不包括路径）。  
    
        :param directory: 要搜索的文件夹路径  
        :param extensions: 要搜索的视频文件扩展名列表，如 ['.mp4', '.avi']  
        :return: 第一个找到的视频文件名（不包括路径），如果未找到则返回None  
        """  
    ...
    def parse_dataset_info(self):
        ...
        #self.real_names = []
        self.fake_names = []
        name=self.find_video_file('samples/manipulated_sequences/videos/c23/videos','.mp4')
        if name is None:
            raise ValueError("No video exists")
        self.fake_names.append(name)
        print(self.fake_names)
        print(f'{self.split} has {len(self.fake_names)} sample(s)')
        
        self.dataset_info =  [[x, 'fake'] for x in self.fake_names]
    ...
    def sample_indices_train(self, video_len):
        ...
        """
        base_idxs = np.array(range(video_len), int)
        if self.sparse_span:
    ...
    def get_enclosing_box(self, img_h, img_w, box, margin):
        ...
        """
        if box is None:  
            raise ValueError("The provided box is None. Cannot compute enclosing box.")
        x0, y0, x1, y1 = box
        w, h = x1 - x0, y1 - y0
        max_size = max(w, h)
    ...
    def __getitem__(self, index):
        ...
        #print(video_name)
        video_face_info_d = self.face_info.get(video_name,None)
        #video_face_info_d='images'
